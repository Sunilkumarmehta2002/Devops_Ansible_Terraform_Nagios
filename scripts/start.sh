...
echo "Starting API via PM2 on api host..."
ANSIBLE_PRIVATE_KEY_FILE="$ANSIBLE_PRIVATE_KEY_FILE" ansible -i "$ROOT_DIR/ansible/inventory.ini" api -m shell -a "export HOME=/home/provertos; cd /opt/provertos/api || exit 0; pm2 startOrRestart ecosystem.config.js"
